...
- 右侧 Dock 固定显示“世界”“轨迹”“档案”。文件读取/预览 API 可以保留，但“文件/预览”Tab 暂时隐藏；未经新的产品评审不得重新暴露。
...
- 推理与运行权限由世界设置统一管理并随世界持久化。权限必须真实传递到 DSH Harness：`read-only` 允许读取和搜索；`workspace-write` 允许调用工具、执行任务并读写当前世界根目录，禁止扩大为主机级无限权限。
