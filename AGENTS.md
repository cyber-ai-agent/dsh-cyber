...
- 聊天双方必须显示头像，用户侧不得用“你”作为唯一身份提示；逐消息推理档位不放在输入工具栏，推理仍由世界或模型设置统一管理。
- 运行权限切换必须真实传递到 DSH Harness。`read-only` 允许读取和搜索；`workspace-write` 允许调用工具、执行任务并读写当前世界根目录，禁止扩大为主机级无限权限。
