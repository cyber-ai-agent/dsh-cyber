...
## 审计整改记录 v2026.09.06-03

新增草稿异步归属护栏，原因：F07 已在真实浏览器复现 A 上传后串入 B；F13 的清空输入不应宣称创建了上下文边界。采用稳定 owner、独立上传代际和真实清空草稿语义，最小浏览器回归加入 required CI。

## 审计整改记录 v2026.09.06-06a

提前落实 F06 固定输入估算拒绝，原因：32,000 字中文请求可经正常入口到达小窗口模型，而原计数会被截小。补充会话重置前预算检查和失败 ID 清理，避免防护本身损坏恢复状态；有效上下文统一与精确来源解释仍在第六批。

## 审计整改记录 v2026.09.06-04a

为 F08 增加原子提交 claim 与 schema 44。请求 key、fingerprint、会话、WorkTurn、owner 消息和可选队列行形成同一事务边界；旧数据无法验证时拒绝猜测。服务器入口尚在接线，不能把此基础提交记作第四批完成。
