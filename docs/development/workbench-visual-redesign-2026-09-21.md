...
- 输入框采用单行起步和自动增高，正文、附件与底部操作带保持清晰分层；聚焦时使用青色边界，发送可用态成为唯一强操作。
- 移除输入框底部的 Enter / Shift+Enter 可见提示，键盘发送、换行和中文输入法确认行为保持原逻辑。
- 附件、消息权限、语音和发送按钮统一高度、圆角、焦点与悬停反馈，明暗主题使用同一套层级。
- 会话行高度收紧至 62px，头像、标题、摘要、时间和行间距同步调整；会话语义与右键管理入口保持原状。
- 世界切换入口按内容宽度排列，箭头紧跟世界名；顶部允许浮层越过工具栏边界，展开菜单完整可见并可点击。
...
- `composer-refinement/redesign-composer-focused.png` / `redesign-composer-dark-focused.png`：输入区亮暗聚焦态。
- `composer-refinement/redesign-world-switcher.png`：紧凑会话列表与完整世界下拉菜单。
...
初版验收使用 Playwright Chromium。本次精修同时使用内置 IAB 和 Playwright Chromium，全部操作通过真实可见控件进行。
