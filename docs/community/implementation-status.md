...
| 连接中心与 SSH 设备 | 顶栏“连接中心”在模型中心之后常驻；Integration Registry 演进为多连接实例宿主（SSH 设备可多实例，单连接类型保留旧流程）；连接凭据走本机 AES-256-GCM 凭据库；设置“外部连接”并入连接中心 | SSH 连接支持新增、编辑、启用、删除和 TCP+SSH banner 探测；私钥只在宿主机内存用于单次会话 | API Token 等服务类型、连接健康历史、凭据轮换 |
