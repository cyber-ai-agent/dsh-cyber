...
本模块记录服务端可观测到的模型 API 交互，用于排查「某次对话、模型发现或知识整理为什么失败、耗时多少、走了哪个模型」等运行问题。日志只存请求摘要统计，**绝不保存 API 密钥、prompt 明文或响应明文**（与 AGENTS.md 凭据红线一致）。

## 采集点与观测边界（必读）

模型 API 的实际请求发生在 DSH worker 进程内部（harness-adapter 的 model-router / adapter），服务端**无法逐请求观测**。因此本模块在三个服务端可见的位置采集，并如实标注边界：
...
| 世界知识整理 | `knowledge` | 后台语义提取使用的模型 ID、成功或失败、耗时、输入/输出字符数与供应商返回的真实 Token | 不创建角色或 AgentRun；不保存来源正文、提取提示和模型原始响应 |
...
2. **`durationMs` 按来源解释**：`turn` 级是**整轮延迟**（从服务端发起 turn 到 worker 返回，含全部工具调用）；`discovery` 是模型目录的单次 HTTP 往返；`knowledge` 是一次后台语义提取与严格结果校验。前端用中文来源徽标区分，做报表或性能分析时三类必须分开。
...
- 约束：`source` ∈ turn/discovery/knowledge，`status` ∈ success/failed，计数与耗时字段非负。
...
- 列表：时间 / 模型 ID + provider / source 徽标（对话回合/模型发现/知识整理）/ 状态（成功/失败）/ 耗时 / token（无则 `—`）。
