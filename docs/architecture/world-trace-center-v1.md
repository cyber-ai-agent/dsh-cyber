...
- 本轮实际发布的产物，以及可以打开它的产物中心链接；
- 失败时可理解的中文说明。

判断摘要只在运行时确实输出过可公开摘要时出现。运行时没有给出摘要时，这一段不显示，也不用任何占位文案代替；轨迹不会替模型编写「当时在想什么」。

产物来自 `WorldArtifactVersion` 的登记，按 `agentRunId` 归属，缺少运行号时退回到该版本登记的工作回合。没有任何登记的版本不会被挂到任何运行上。轨迹只携带产物的标识、标题、类型和版本号，预览与内容始终属于产物中心。

每条角色运行轨迹都带上持久化的 `AgentRun` 标识（`runId`），上下文视图和审计可以据此回到同一次运行，而不必反解卡片 ID。
...
结果按最新时间倒序返回。日期按运行应用的本地日历解释，关键词可匹配轨迹摘要、判断摘要、角色名、工具名称、产物标题、模型和服务。游标用于继续读取更早的轨迹。
...
- 缺失的判断摘要显示为缺失，不会被占位文案填充；
- 运行发布的产物挂到正确的运行上，没有产物的运行不显示产物；
- 聊天界面不展示推理与工具明细。
