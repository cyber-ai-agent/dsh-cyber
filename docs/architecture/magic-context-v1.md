...
确定性前缀现在由 `ContextLayerCache` 做进程内分层复用。缓存只保存可由持久事实重建的 `ContextLayer`，以 `kind/id/revision/contentHash/sourceRefs` 内容寻址，并使用有界 LRU；请求、实时工具结果和记忆排序不会进入缓存。角色修订、世界规则、名册或机器档案变化会产生新的内容键，服务重启只会清空缓存，不会影响 SQLite、记忆索引或会话历史。Provider 的 `cachedPrefixTokens` 仍由具体适配器上报，未上报时检查器继续显示 `unavailable`。
...
## D2.5 Provider 前缀缓存 / D3 上下文检查器 / D4 上下文快照

这三片与本文档并行开发，**不在本分支上**。本节只记录它们要落进的接缝，不替它们宣称结果。

- **D2.5 Provider 前缀缓存**：`stableContextHash` 就是给模型服务商适配器的缓存键。适配器可以把它映射到自己的 prompt-cache 协议；没有真实命中统计时，基准里的 `cachedPrefixTokens` 保持 `null`。这与上面的宿主分层缓存是两层边界，不能用宿主命中次数冒充服务商命中 token。
