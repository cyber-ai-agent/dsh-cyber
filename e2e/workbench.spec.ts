...
test('creates, edits, restores, and deletes a private-network model provider in the model hub', async ({ page }) => {
  await page.setViewportSize({ width: 1_920, height: 1_080 })
  await page.route('**/api/workspaces/*/model-providers/*/test', async (route) => {
    await route.fulfill({
      status: 200,
  ...
  let hub = page.getByRole('dialog', { name: 'AI 模型管理中心' })
  await expect(hub).toBeVisible()
  await hub.getByRole('button', { name: '添加服务商', exact: true }).click()
  const wizard = hub.getByRole('region', { name: '添加或编辑服务商' })
  await wizard.getByRole('combobox').first().selectOption('local')
  await wizard.getByRole('textbox', { name: '名称' }).fill('公司 sub2api')
  await wizard.getByRole('textbox', { name: /接口地址/ }).fill('http://127.0.0.1:11434/v1/')
  await wizard.getByRole('textbox', { name: 'API 密钥', exact: true }).fill('sk-e2e-test-only-not-real')
  await wizard.getByRole('button', { name: '测试服务商并获取模型列表' }).click()
  await expect(wizard.getByText('获取到 3 个模型')).toBeVisible()
  await expect(wizard.getByRole('checkbox')).toHaveCount(3)
  await wizard.getByRole('button', { name: '保存并导入模型池' }).click()

  const providerCard = hub.locator('.model-hub__provider-card').filter({ hasText: '公司 sub2api' })
  await expect(providerCard).toBeVisible()
  await expect(providerCard).toContainText('3 个模型')

  await providerCard.getByRole('button', { name: '编辑', exact: true }).click()
  const editor = hub.getByRole('region', { name: '添加或编辑服务商' })
  await expect(editor.getByRole('textbox', { name: '名称' })).toHaveValue('公司 sub2api')
  await editor.getByRole('textbox', { name: '名称' }).fill('公司 sub2api 编辑')
  await editor.getByRole('button', { name: '测试服务商并获取模型列表' }).click()
  await expect(editor.getByText('获取到 3 个模型')).toBeVisible()
  await editor.getByRole('button', { name: /完成（服务商已保存/ }).click()
  await expect(hub.locator('.model-hub__provider-card').filter({ hasText: '公司 sub2api 编辑' })).toBeVisible()
  ...
  await settings.getByRole('button', { name: /AI 模型/ }).click()
  await settings.getByRole('button', { name: '打开模型中心', exact: true }).click()
  hub = page.getByRole('dialog', { name: 'AI 模型管理中心' })
  const restored = hub.locator('.model-hub__provider-card').filter({ hasText: '公司 sub2api 编辑' })
  await expect(restored).toBeVisible()
  await expect(restored).toContainText('密钥')
  await page.setViewportSize({ width: 1_920, height: 1_080 })
  ...
  await restored.getByRole('button', { name: /删除服务商/ }).click()
  await hub.getByRole('button', { name: '确认删除', exact: true }).click()
  await expect(hub.locator('.model-hub__provider-card').filter({ hasText: '公司 sub2api 编辑' })).toHaveCount(0)
})
