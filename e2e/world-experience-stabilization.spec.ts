...
test('shows existing role instances and warns before creating a duplicate name', async ({ page }) => {
  ...
  await market.getByRole('button', { name: /管家 v1/ }).click()
  await expect(market.getByText('当前世界已有 1 名')).toBeVisible()
  await expect(market.getByText(/当前世界已经有 1 名角色来自这份模板：管家/)).toBeVisible()

  const name = market.getByLabel('角色名字（可选）')
  await name.fill('管家')
  await expect(market.getByText('当前世界已有同名角色，建议换一个名字以便区分。')).toBeVisible()
  await expect(market.getByRole('button', { name: '再创建一名' })).toBeEnabled()
  await market.getByRole('button', { name: '取消' }).click()
})
