...
test('runs the task, immutable deliverable review, change request, and accepted replacement flow', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '自动组队并开工' }).click()
  await expect(page.locator('.task-status')).toHaveText('等待验收', { timeout: 15_000 })
  await expect(page.getByText('任务群聊已创建，已邀请 2 名角色加入并开始分工。')).toBeVisible()
  await expect(page.getByRole('button', { name: '进入任务群聊' })).toBeVisible()
  ...
  await expect(page.locator('.task-status')).toHaveText('等待验收', { timeout: 15_000 })
  expect(current.store.listSessions(world.id).filter((item) => item.collaborationMode === 'task')).toHaveLength(1)
