...
import type { ConversationQueueEntry, JsonObject, WorkMessage } from '@dsh-cyber/contracts'
...
export function composeConversationControl(options: {
  ...
  const queue = new ConversationQueueService({
    ...
    runner: async (entry) => {
      ...
      await runQueuedGroup(entry, options)
    },
...
async function runQueuedGroup(
  ...
  options: Pick<Parameters<typeof composeConversationControl>[0], 'store' | 'orchestrator' | 'groupTasks' | 'worldPackages' | 'runtimeContext'>,
): Promise<void> {
  ...
  await options.orchestrator.group({
    workspaceId: entry.workspaceId,
    worldId: entry.worldId,
    employeeIds: entry.employeeIds,
    prompt,
    ...
    sessionId: entry.sessionId,
    existingWorkTurnId: entry.workTurnId,
    runtimePrompt: await options.runtimeContext.composeGroupRuntimePrompt(entry.worldId, transformedPrompt),
    ...(entry.reasoningEffort === undefined ? {} : { reasoningEffort: entry.reasoningEffort }),
    ...(entry.permissionMode === undefined ? {} : { permissionMode: entry.permissionMode }),
  })
}
