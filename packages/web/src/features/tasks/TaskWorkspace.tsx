...
function WorldTaskWorkspace({ world, employees, onOpenSession }: TaskWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    return subscribeWorldLiveRefresh(world.id, ['world-task', 'world-artifact'], load)
  }, [load, world.id])
  useEffect(() => {
...
function TaskDetail({ detail, employees, artifacts, busy, mutate, onOpenSession }: { detail: WorkTaskDetail; employees: CyberEmployee[]; artifacts: WorldArtifact[]; busy: boolean; mutate(operation: () => Promise<unknown>): Promise<void>; onOpenSession(sessionId: string): Promise<void> }) {
  ...
  const sourceTurnWorking = detail.sourceTurn !== undefined && ['queued', 'running', 'waiting-approval'].includes(detail.sourceTurn.status)
  ...
  return <>
    ...
    {!sourceTurnWorking && (CANCELLABLE.includes(detail.task.status) || (sourceOnly && detail.task.status === 'waiting-review')) ? <CancelTask taskId={detail.task.id} busy={busy} mutate={mutate} /> : null}
  </>
}
