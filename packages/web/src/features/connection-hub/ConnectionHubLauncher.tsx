...
export function ConnectionHubLauncher({ workspace, onClosed }: { workspace: Workspace; onClosed?(): void }) {
  ...
  return <>
    <button type="button" aria-label={t('app.connectionHub', '连接中心')} title={t('app.connectionHub', '连接中心')} aria-haspopup="dialog" onClick={() => setOpen(true)}><PlugsConnected size={16} /><span>{t('app.connectionHub', '连接中心')}</span></button>
