...
export function ChatWorkbench({ demoMode, world, session, intent, participantIds = [], messages, employees, liveTurns, sending, draft, reasoningEffort = 'auto', onReasoningEffortChange = () => undefined, onDraftChange, onSend, onUploadAttachment, onOpenDossier, onOpenArtifact, onRecruit }: ChatWorkbenchProps) {
    ...
    <section className="chat-workbench" aria-label="当前世界多角色会话">
      ...
      <div className="message-scroll" ref={scrollRef} aria-live="polite" aria-busy={sending}>
        {messages.length === 0 ? (
          ...
          if (message.kind === 'reasoning') return <ReasoningMessage key={message.id} message={message} employee={employee} />
          if (message.kind === 'tool-call' || message.kind === 'tool-result') return <ToolEventMessage key={message.id} message={message} employee={employee} />
          return (
        ...
        {liveTurns.length > 0 ? <div className="live-turns-block" aria-label="实时执行过程"><header className="live-turns-block__header"><TerminalWindow size={14} /><span>实时执行过程</span></header>{liveTurns.map((turn) => <LiveTurn key={`${turn.sessionId}:${turn.agentId}`} turn={turn} employee={employees.find((item) => item.id === turn.agentId)} />)}</div> : null}
        {sending && liveTurns.length === 0 ? <div className="stream-state"><CircleNotch size={16} className="spin" /><span>正在连接{experience.personLabel}的独立运行时…</span><span className="stream-caret" /></div> : null}
      </div>
...
function LiveTurn({ turn, employee }: { turn: LiveAgentTurn; employee: CyberEmployee | undefined }) {
  ...
  return <article className={`live-turn live-turn--${turn.status}${live ? ' live-turn--live' : ''}`}><header><span>{turn.status === 'failed' ? <WarningCircle size={16} /> : <CircleNotch size={16} className={live ? 'spin' : ''} />}</span><div><strong>{employee?.displayName ?? '角色'}</strong><small>{liveStatusLabel(turn.status)}</small></div>{live ? <em className="live-turn__badge">实时</em> : null}</header>{turn.reasoning ? <div className="trace-stack"><button className="trace-disclosure" type="button" onClick={() => setReasoningOpen((value) => !value)}><span><TerminalWindow size={15} />实时思考过程</span><CaretDown size={14} className={reasoningOpen ? 'is-open' : ''} /></button>{reasoningOpen ? <p className="live-turn__reasoning">{turn.reasoning}</p> : null}</div> : null}{turn.tools.length > 0 ? <ToolTimeline tools={turn.tools} /> : null}{turn.text ? <p className="live-turn__text">{turn.text}<span className="stream-caret" /></p> : null}</article>
}

function ReasoningMessage({ message, employee }: { message: WorkMessage; employee: CyberEmployee | undefined }) { return <details className="reasoning-message"><summary><span><CircleNotch size={14} />{employee?.displayName ?? '角色'}的思考过程</span><span>{displayTime(message)}</span></summary><div>{message.content}</div></details> }
...
function ToolEventMessage({ message, employee }: { message: WorkMessage; employee: CyberEmployee | undefined }) {
  ...
  return <div className={`tool-event-message${failed ? ' is-failed' : ''}`}>{failed ? <WarningCircle size={15} /> : started ? <CircleNotch size={15} className="spin" /> : <CheckCircle size={15} weight="fill" />}<span><strong>{employee?.displayName ?? '角色'} · {started ? '调用工具' : '工具结果'}</strong><code>{toolName}</code></span><small>{callId}</small></div>
}
...
function ToolTimeline({ tools }: { tools: ToolStep[] }) { return <ol className="tool-timeline">{tools.map((tool) => <li key={tool.id} className={`tool-step tool-step--${tool.status}`}><span className="tool-step__rail" />{tool.status === 'complete' ? <CheckCircle size={16} weight="fill" /> : <CircleNotch size={16} className="spin" />}<span><strong>{tool.label}</strong><code>{tool.target}</code></span><time>{tool.duration ?? '运行中'}</time></li>)}</ol> }
function liveStatusLabel(status: LiveAgentTurn['status']): string { return ({ thinking: '正在思考', working: '正在调用工具', completed: '本轮完成', failed: '执行失败' })[status] }
