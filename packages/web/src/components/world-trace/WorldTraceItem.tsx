...
export function WorldTraceItem({ entry, employees, open, onToggle, onOpenArtifact, onOpenContext }: {
  ...
  return <li className={`world-trace-item world-trace-item--${entry.status}`}>
    ...
    {!hasDetails ? <article>{content}</article> : <article className="world-trace-item__expandable"><details open={open} onToggle={(event) => onToggle(event.currentTarget.open)}><summary><div className="world-trace-item__summary">{content}</div><span className="world-trace-item__expand-label">查看过程<CaretDown size={14} /></span></summary><div className="world-trace-item__detail">
      ...
      {entry.tools?.length ? <section><strong>工具调度</strong><ol className="world-trace-tools">{entry.tools.map((tool) => <li key={tool.callId} className={`is-${tool.status}`}>{tool.status === 'running' ? <CircleNotch size={14} className="spin" /> : tool.status === 'failed' ? <WarningCircle size={14} /> : <CheckCircle size={14} weight="fill" />}<span><strong>{tool.label}</strong>{tool.description ? <small>{tool.description}</small> : null}{tool.input ? <code className="world-trace-tools__input" title={tool.input}>{tool.input}</code> : null}{tool.name ? <code>{tool.name}</code> : null}</span><small>{tool.status === 'running' ? '执行中' : tool.status === 'failed' ? '失败' : '完成'}{tool.durationMs === undefined ? '' : ` · ${formatDuration(tool.durationMs)}`}</small></li>)}</ol></section> : null}
