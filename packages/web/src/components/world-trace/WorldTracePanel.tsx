...
export function WorldTracePanel({ world, employees, demoMode, onOpenArtifact }: {
  ...
  return <section className="world-trace-panel" aria-label={`${world.name} - ${t('workbench.traceTitle', '世界轨迹')}`}>
    <header className="world-trace-panel__header"><span className="world-trace-panel__mark"><Path size={20} /></span><span><strong>{t('workbench.traceTitle', '世界轨迹')}</strong><small>{t('workbench.traceSubtitle', '可读的判断摘要、工具调度与实际用量')}</small></span><button type="button" className="icon-button" aria-label={t('workbench.traceRefresh', '刷新世界轨迹')} disabled={trace.loading} onClick={() => void trace.refresh()}><ArrowClockwise size={17} className={trace.loading ? 'spin' : ''} /></button></header>
