    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="recruitment-dialog" role="dialog" aria-modal="true" aria-labelledby="recruitment-title">
        <header className="dialog-header">
          <div><h2 id="recruitment-title">{experience.marketLabel}</h2><p>添加后会创建当前世界专属的独立角色。每个角色拥有自己的设定、会话、记忆与成长记录。</p></div>
        ...
        <div className="recruitment-layout">
          ...
          <div className="blueprint-detail">
            {selected === undefined ? null : <>
              ...
              {selectedExisting.length > 0 ? <div className="permission-notice permission-notice--existing"><IdentificationCard size={18} /><p>当前世界已经有 {selectedExisting.length} 名角色来自这份模板：{selectedExisting.map((employee) => employee.displayName).join('、')}。仍可创建新的独立角色实例。</p></div> : null}
              ...
              <div className="permission-notice"><ShieldCheck size={18} /><p>创建角色不会自动扩大权限。需要额外访问文件、知识或工具时，仍按最小权限逐项批准。</p></div>
            </>}
          </div>
        </div>

        <footer className="dialog-footer"><span>{selected === undefined ? '请选择一份角色模板' : `将在当前世界创建独立角色：${displayName.trim() || selected.displayName}`}</span><div><button className="text-button" type="button" onClick={onClose}>取消</button><button className="primary-button" type="button" disabled={selected === undefined || recruiting} onClick={() => selected && void onRecruit(selected, displayName.trim() || undefined, capabilityGrants)}>{recruiting ? '正在创建角色…' : selectedExisting.length > 0 ? '再创建一名' : roleplay ? '邀请角色入场' : '确认添加'}</button></div></footer>
      </section>
    </div>
  )
}
...
function CapabilityApprovalGroup({ items, selected, onChange }: { items: string[]; selected: string[]; onChange(next: string[]): void }) {
  return <fieldset className="capability-approval-group"><legend>高级权限（按需批准）</legend>{items.length === 0 ? <span>该角色模板未请求额外权限</span> : items.map((item) => <label key={item}><input type="checkbox" checked={selected.includes(item)} onChange={(event) => onChange(event.target.checked ? [...selected, item] : selected.filter((value) => value !== item))}/><code>{item}</code></label>)}</fieldset>
}
