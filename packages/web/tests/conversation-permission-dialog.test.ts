...
describe('ConversationPermissionControl', () => {
  ...
  it('keeps operation permission in the composer menu', async () => {
    ...
    expect(host.textContent).toContain('编辑外部文件和使用互联网时始终询问')
    expect(host.textContent).toContain('仅对检测到的风险操作请求批准')
    expect(host.textContent).toContain('可不受限制地访问互联网和你电脑上的任何文件')
